...
$itemName = $description = $category = $subcategory = $price = ""; 
$imagePath = "";
$errors = [];
        ...
        if ($_FILES['itemImage']['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($fileType, $allowTypes)) {
                if (move_uploaded_file($_FILES["itemImage"]["tmp_name"], $targetFilePath)) {
                    $sql = "INSERT INTO productsservices (itemName, description, category, subcategory, price, imagePath) VALUES (?, ?, ?, ?, ?, ?)";
                    $stmt = $conn->prepare($sql);
                    if ($stmt->execute([$itemName, $description, $category, $subcategory, $price, $targetFilePath])) {
